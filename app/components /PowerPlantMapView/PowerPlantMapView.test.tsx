...
jest.mock('next/dynamic', () => (fn: () => Promise<{ default: React.ComponentType<Record<string, unknown>> }>) => {
...
describe('PowerPlantMapView', () => {
  it('renders without crash', () => {
    mockUseGetPowerPlants.mockReturnValue({ plants: undefined, error: null, isLoading: false });
  ...
  it('shows prompt when no country selected', () => {
    mockUseGetPowerPlants.mockReturnValue({ plants: undefined, error: null, isLoading: false });
    render(<PowerPlantMapView country='' />);
    expect(screen.getByText('Select a country to view power plants')).toBeDefined();
  });

  it('shows loading state when country given', () => {
    mockUseGetPowerPlants.mockReturnValue({ plants: undefined, error: null, isLoading: true });
    render(<PowerPlantMapView country='Germany' />);
    expect(screen.getByText('Loading map...')).toBeDefined();
  });

  it('shows error state', () => {
    mockUseGetPowerPlants.mockReturnValue({ plants: undefined, error: new Error('fail'), isLoading: false });
    render(<PowerPlantMapView country='Germany' />);
